...
#
# KR5194450018 W클래스(51쪽): 이 문서에 "종류W(랩)"는 실존하는
# 클래스지만(9/17/28/30/35/37/38쪽 보수표·설명에 계속 등장), 연도별
# 수익률표(5·50쪽)엔 애초에 W가 없다(A/A-e/C1~C4만 있음 - 거래 이력이
# 없는 클래스로 보인다). 그런데 51쪽 원문을 직접 열어보면 그 자리는
# "다. 집합투자기구의 자산 구성 현황"표(자산총액·자산군별 비중)라,
# 수익률과 아예 무관하다. 파서가 이 표의 숫자를 W클래스 연도별
# 수익률로 잘못 집었다 - 4년차 값 90.0은 이 표의 "자산총액 90(억원)"
# 칸, 나머지 0.0들은 채권 외 자산군 비중 "0.00" 칸이다.
_KNOWN_FAKE_ROWS = {
    ("KR5153420318", "fund", None, 2),
    ("KR5153420318", "benchmark", None, 2),
    ("KR5194450018", "class_return", "W", 1),
    ("KR5194450018", "class_return", "W", 2),
    ("KR5194450018", "class_return", "W", 3),
    ("KR5194450018", "class_return", "W", 4),
    ("KR5194450018", "class_return", "W", 5),
}
