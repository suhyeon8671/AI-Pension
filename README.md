...
비교지수/변동성 조합) 전부 원본 페이지와 육안 대조 일치 확인. 전체 실행
결과 **547개 행(94개 문서)** — class_return 333건(클래스코드 인식 147건),
benchmark/volatility 214건. 값 범위(-25.77~99.84%)도 상식적인 범위 안.

### AUM(운용규모) — 데이터 자체가 없음, 포기

대회 자료의 상품 6축(상품분류/위험등급/판매클래스/총보수/수익률/AUM) 중
AUM만 보류했다. products 100개 문서 전체(텍스트+표)를 "순자산총액"/
"설정액"/"설정원본"/"운용규모" 키워드로 뒤졌는데, 나오는 건 전부 (1) 기준
가격 산정 방식을 설명하는 법적 정의 문구, 아니면 (2) "설정액 50억원 미만이면
소규모펀드"라는 경고 문구(286건 검색됐는데 전부 이 하나의 반복)뿐이었다.
**이 상품 자체의 실제 운용규모 숫자는 간이투자설명서 어디에도 없다** —
간이투자설명서는 정기 갱신되는 정적 문서라 매일 바뀌는 AUM은 안 싣고
"금융투자협회 홈페이지(dis.kofia.or.kr)에서 확인하라"고만 안내한다.

(참고: "운용전문인력" 표에 있는 "운용규모(억원)"는 이 상품 것이 아니라
**펀드매니저 한 명이 운용하는 전체 펀드 합산 규모**라 완전히 다른 숫자다 -
헷갈려서 잘못 쓰면 안 된다.)

AUM을 실제로 채우려면 PDF 추출이 아니라 금융투자협회 공시 웹사이트를
스크래핑/API 호출하는 완전히 다른 작업이 필요해서, 이번 범위에서는 보류하고
`product_name`/`asset_type`/`risk_level`/`class`/`total_fee`/`return`
5축까지만으로 마무리하기로 했다.
