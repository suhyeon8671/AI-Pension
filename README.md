...
1차 실행 결과 125개 클래스 레코드(31개 문서)를 뽑았고, 6개 문서는 총보수
값은 잡았지만 클래스 코드(괄호 텍스트) 인식에 실패해 confidence 0.5로
낮춰 저장했다. `sales_commission_desc`는 125건 중 94건(75%) 인식, 나머지는
문구 변형(정규식 미대응)으로 null. **이건 "총보수 표"만 대상으로 한 결과이고,
